...
#这个图的x轴刻度有点少，我稍微增加了一些，我还把每个bar的width搞得小了一点（修改图在下面）。
...
ggplot(film, aes(x = length, y = rating)) +
  ...
  geom_hline(yintercept = 7, linetype = "dashed", color = "red", size = 1)
# 通过这个图我们可以看出，所有的数据其实分成了两部分，一部分是短片，一部分是长片。所以我觉得可以新设立一个变量为long_film，低于40分钟为短片，高于40分钟为长片。在genre里也有short这个分类，但其实有被分到其他类别的电影也低于40分钟，甚至只有几分钟。在下面的代码块里我展示了低于40分钟的电影在不同的genre里的数量。
...
film %>%
  filter(length < 40) %>%
  count(genre)
# 设新的变量的代码如下，如果设了这个变量之后，后续的代码需要微调：
...
# film <- film %>% mutate(long_film = ifelse(length < 40, "short", "long"))
```
...
#这个matrix没有相关系数的数值，不是很直观，因此我在下面这个版本的matrix里同时有颜色和数值（修改图在下面）。
...
```{r}
# 这个代码框是加上了新的变量（long_film) 之后的新模型，可以对比一下之前模型。
# This code block presents the new model after adding the long_film variable, so you can compare it with the previous model.
new_model <- glm(rating_binary ~ length + budget + votes + genre + long_film, data = film, family = binomial)
```
